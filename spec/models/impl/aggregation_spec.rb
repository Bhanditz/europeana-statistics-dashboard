...
RSpec.describe Impl::Aggregation, type: :model do
  ...
  context '#get_ga_data' do
    it 'should return Google Analytics data for given metric an dimension sorted by given order' do
      data_provider = impl_aggregations(:locloud_aggregation)
      start_date = '2016-03-01'
      end_date = '2016-03-02'
  ...
  context '#get_data_providers_json' do
    it 'should return the data providers data as json' do
      expected_data = { 'url' => 'http://localhost:3000/dataprovider/content-2014', 'text' => "Diputació de Barcelona" }

      data = Impl::Aggregation.get_data_providers_json.first
      expect(data['url']).to eq(expected_data['url'])
      expect(data['text']).to eq(expected_data['text'])
    end
  end

  context '#get_providers_json' do
    it 'should return the providers data as json' do
      expected_data = { 'url' => 'http://localhost:3000/provider/content-2013', 'text' => 'LoCloud' }

      data = Impl::Aggregation.get_providers_json.first
      expect(data['url']).to eq(expected_data['url'])
      expect(data['text']).to eq(expected_data['text'])
    end
  end
  ...
  context '#get_data_providers_count_query' do
    let(:aggregation) {impl_aggregations(:locloud_aggregation)}
    it 'should return a query that get data_provider count' do
      query =  "Select count(*) as value, '' as key, '' as content, 'Total Institutions' as title, '' as diff_in_value from impl_aggregation_relations where impl_parent_genre='provider' and impl_child_genre='data_provider' and impl_parent_id = '#{aggregation.id}'"
