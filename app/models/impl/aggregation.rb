...
class Impl::Aggregation < ActiveRecord::Base
  ...
  validates :genre, presence: true, inclusion: { in: %w(provider data_provider country europeana) }
  ...
  def get_aggregated_filters
    ...
    raise 'No data set' if impl_data_sets.count == 1 && Constants::FILTERED_280_DATASETS.include?(impl_data_sets.first.name)

    impl_data_sets.each do |p|
      filter += "ga:pagePath=~/#{p.data_set_id.strip}/," unless Constants::FILTERED_280_DATASETS.include?(p.name) && genre == 'data_provider'
    end
    filter[0..-2]
  end
  ...
  def blacklist_data_set?
    (genre == 'data_provider' && (impl_data_sets.pluck(:name).uniq & Impl::BlacklistDataset.get_blacklist_datasets).present?)
  end
  ...
  def self.fetch_GA_data_between(start_date, end_date, data_provider = nil, extra_dimension, metric)
    ...
    if data['totalResults'].to_i > 0
      data = data['rows']
      if data.present?
        data = data.map { |a| { 'month' => a[0], 'year' => a[1], extra_dimension.to_s => a[2], metric.to_s => a[3].to_i } }
  ...
  def get_data_providers_count_query
    "Select count(*) as value, '' as key, '' as content, 'Total Institutions' as title, '' as diff_in_value from impl_aggregation_relations where impl_parent_genre='#{genre}' and impl_child_genre='data_provider' and impl_parent_id = '#{id}'"
  end
  ...
  def get_aggregations_count_query(genre = nil)
    return '' if self.genre != 'europeana' && %w(country provider data_provider).include?(genre)
    today = Date.today
