...
class Impl::AggregationRelation < ActiveRecord::Base
  ...
  validates :impl_parent_genre, presence: true, inclusion: { in: %w(country provider) }
  validates :impl_child_id, presence: true
  validates :impl_child_genre, presence: true, inclusion: { in: %w(provider data_provider) }
