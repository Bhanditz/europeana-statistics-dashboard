...
class Core::Viz < ActiveRecord::Base
  ...
  scope :item_view_line_chart, -> {where("core_vizs.name LIKE '%- Item View Line Chart'")}
  scope :countries_count, -> {where("core_vizs.name LIKE '% - Countries Count'")}
  scope :providers_count, -> {where("core_vizs.name LIKE '% - Providers Count'")}
  scope :data_providers_count, -> {where("core_vizs.name LIKE '% - Data Providers Count'")}
  #scope :working_media_link, -> {where("core_vizs.name LIKE")}
  scope :manual, ->{where(is_autogenerated: false)}
  scope :auto_generated, -> {where(is_autogenerated: true)}
