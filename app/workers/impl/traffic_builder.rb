class Impl::TrafficBuilder
  ...
  def perform(provider_id, user_start_date = "2012-01-01",user_end_date = (Date.today.at_beginning_of_week - 1).strftime("%Y-%m-%d"))
    ...
    provider_events_output = Impl::Output.find_or_create(provider_id,"Impl::Provider","events")
    ...
    begin
      ...
      provider_pageviews_output.update_attributes(status: "Failed", error_messages: e.to_s)
      provider.update_attributes(status: "Failed", error_messages: e.to_s)
    end
    #Events
    begin
      provider_pageviews_output.update_attributes(status: "Building Events", error_messages: nil)
