class Impl::DataProviders::TopDigitalObjectsBuilder
  ...
  def perform(data_provider_id)
    ...
    begin
      ...
      top_digital_objects = Impl::DataProviders::TopDigitalObjectsBuilder.fetch_data_for_all_quarters_between(aggregation.last_updated_at.strftime("%Y-%m-%d") || "2012-01-01", (Date.today.at_beginning_of_week - 1).strftime("%Y-%m-%d"), data_provider)
      ...
      data_provider.update_attributes(status: "Built top digital objects", last_updated_at: Date.today)
    rescue => e
  ...
  def self.fetch_data_for_all_quarters_between(start_date, end_date, data_provider)
    ...
    ga_max_results = data_provider.last_updated_at.present? ? 100 : 1000
