...
class Impl::ReportsController < ApplicationController
  ...
  def show
    ...
    begin
      gon.top_digital_objects = JSON.parse(Nestful.get(REST_API_ENDPOINT+"datacast/"+@impl_aggregation.core_datacasts.top_digital_objects.first.identifier).body)
    rescue => e
      gon.top_digital_objects = []
    end
  end
