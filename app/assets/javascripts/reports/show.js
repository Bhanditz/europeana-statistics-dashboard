...
Rumali.buildCharts = (function () {
	...
	for(;index<charts_length;index++){
		...
		switch(api_data){ //calling function to render the chart as per the data attribute value.
			...
			case 'PykCharts.maps.oneLayer':
				...
				loadTopCountryCard(chart_data.charts[index],chart_data.year_no,chart_data.quarter_no);
				break;
			default:
				break;
		}
	...
	var loadTopCountryCard = function(obj,year,quarter){
		...
		var filterData = function(data,year,quarter){
			...
			renderHTMLForCard('holdy',title,country_name,total_views,content);
	...
	var renderHTMLForCard = function(id,title,key,value,content){
		...
		htmlcontent += '<span class= col-sm-12 style="text-align:center;"><h4 style="color: white;margin-bottom:10px;"><b>'+title+'</b></h4></span>';
		htmlcontent += '<span class= col-sm-12 style="text-align:center"><b>'+key+'</b></span>';
		htmlcontent += '<span class= col-sm-12 style="text-align:center"><b>'+value+'</b></span>';
		htmlcontent += '<span class= col-sm-12 style="text-align:left;font-size:12px;">'+content+'</span>';
		htmlcontent += '</span>';
